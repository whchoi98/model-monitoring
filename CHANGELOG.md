...
### Infra
- New EventBridge schedule `FeaturesVerifySchedule` (`rate(24 hours)`) → Fargate `FeaturesVerifyTaskDef` (`python -m features_runner --once`, log group `/ecs/features`, autoprober task role reused — no new IAM). Env `MANTLE_ANTHROPIC_REGION=us-east-1` and `FEATURES_MCP_SERVER_URL` injected into all scheduler tasks and the backend service. One run = 658 probes + 122 pre-decided rows = 780 cells (39 features × 5 surfaces × 4 models); with 2-call caching probes and negative controls, ≈800 API calls. Daily cadence approved by the user on 2026-09-05 for the 4-surface design (estimated $4–6/run); adding the Bedrock Messages API surface raises the estimate to ≈$5–7/run — cost re-approval pending.
- 신규 EventBridge 스케줄 `FeaturesVerifySchedule`(`rate(24 hours)`) → Fargate `FeaturesVerifyTaskDef`(`python -m features_runner --once`, 로그 그룹 `/ecs/features`, autoprober 태스크 롤 재사용 — 신규 IAM 없음). env `MANTLE_ANTHROPIC_REGION=us-east-1`·`FEATURES_MCP_SERVER_URL`을 스케줄 태스크 전체와 backend 서비스에 주입. 1런 = 프로브 658 + 사전판정 122 = 780셀(39피처 × 5 surface × 4모델), 캐싱 프로브·부정 제어 포함 ≈800 API 호출. 일 1회 주기는 2026-09-05에 4 surface 설계(런당 추정 $4~6) 기준으로 사용자 승인을 받았고, Bedrock Messages API surface 추가로 추정이 런당 ≈$5~7로 올라간다 — 비용 재승인 대기.
...
- Sonnet 5 full sweep (39 features × 5 surfaces): **broken 0 / inconclusive 0** — every cell lands on a decided status. These were **two separate runs**: the 4-surface sweep after the Task 7 probe fixes (its Mantle column was measured on `ap-northeast-1`, i.e. before the region switch to `us-east-1`, so those cells read `unsupported` for model-not-served rather than broken), and the `bedrock_messages` sweep on 2026-09-05 after the route-403 normalization. On `bedrock_messages`, 22 supported / 15 unsupported / 1 skipped (1M context) / 1 not_applicable (`extended_thinking`, adaptive-only model), with 35 `match`, 1 `undocumented` (`browser_use` — it also works here) and no drift. `token_counting`, `models_api`, `files_api` and `batch_processing` are cleanly absent: the first three answer with a coral `UnknownOperationException`, and `/v1/messages/batches` falls through to the SigV4 front door (403 "Authorization header is missing") because the two auth schemes are mutually exclusive — the transport records both as route absence. `tool_search` works here even though AWS documents it for InvokeModel only (expectation left `unknown`).
- Sonnet 5 전체 스윕(39피처 × 5 surface): **broken 0 / inconclusive 0** — 모든 셀이 판정 가능한 상태로 수렴한다. 단 **서로 다른 두 런**이다: Task 7 프로브 수정 후의 4 surface 스윕(Mantle 열은 리전 전환 이전 `ap-northeast-1`에서 측정 — 모델 미서빙이 broken이 아니라 `unsupported`로 수렴)과, 라우트 403 정규화 후의 2026-09-05 `bedrock_messages` 스윕. `bedrock_messages`는 supported 22 / unsupported 15 / skipped 1(1M 컨텍스트) / not_applicable 1(`extended_thinking` — adaptive 전용 모델)이고, 판정은 `match` 35 · `undocumented` 1(`browser_use`가 이 경로에서도 동작) · 드리프트 0. `token_counting`·`models_api`·`files_api`·`batch_processing`은 깨끗하게 부재로 확인 — 앞 3개는 coral `UnknownOperationException`, `/v1/messages/batches`는 SigV4 프론트도어로 떨어져 403 "Authorization header is missing"(두 인증 스킴이 배타적이라 헤더 추가로 재측정 불가) → 전송기가 둘 다 라우트 부재로 기록한다. `tool_search`는 AWS 문서가 InvokeModel만 명시하는데도 이 경로에서 동작(기대치 `unknown` 유지).
