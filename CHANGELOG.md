...
- **Shared `AppHeader` no longer collapses in EN at 1440px** — the right group shrinks and the desktop nav scrolls horizontally instead of squeezing the brand to zero width; the subtitle truncates instead of wrapping one word per line (the header grew from 56px to 158px with the KO/EN toggle rendered inside the subtitle).
- **공용 `AppHeader`가 EN 1440px에서 무너지지 않음** — 브랜드를 폭 0으로 누르는 대신 오른쪽 그룹이 줄어들고 데스크톱 내비가 가로 스크롤; 부제는 한 단어씩 줄바꿈하지 않고 말줄임(헤더가 56px→158px로 커지고 KO/EN 토글이 부제 안에 그려지던 문제).
