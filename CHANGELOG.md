...
### Added
- **Unit Prices page (`/pricing`, "비용 단가")** (user request 2026-09-26). One row per model family and one column per channel (Claude Platform on AWS, Global, US, In-Region) show the Standard input and output price per 1M tokens (`$4.00 / $20.00`). Every price carries a numbered footnote that jumps to its entry in the reference list, and the page downloads as CSV, Markdown or JSON (`/api/pricing/export?format=csv|md|json&lang=ko|en`, file `llm-monitor-unit-prices-YYYY-MM-DD.<ext>`). An amber box at the top and a line after the references say "This price list is compiled automatically from public sources for reference only and is not an official AWS statement. Always confirm final prices on the official pricing pages.", and every download carries the same text. Cells get a "Not auto-verified" badge when the latest finished sync did not observe them, "Pending review" when a large change waits for approval, and GPT-5.6 Sol gets a manual promotion note (at least until 2026-11-21) that turns into "check whether the promotion ended" after that date. Each badge's explanation (the last-checked date or "initial value", the new value waiting for review, the price before the promotion with a footnote to the manual note) is plain text next to the badge rather than a hover tooltip, so touch and keyboard users see it too, and a "Show model IDs" toggle lists the model IDs behind each price. On a phone only the table scrolls sideways; a hint above each table and a fade on its right edge say so, because the first visible price column is Claude Platform on AWS and an OpenAI or Nova row would otherwise look empty. The menu item sits right after Cost. The table covers the 19 families of the 55 active channels; dormant 1P and hidden channels are excluded.
- **비용 단가 페이지(`/pricing`, "Unit Prices")**(2026-09-26 사용자 요청). 모델 패밀리마다 한 행, 채널(Claude Platform on AWS, Global, US, In-Region)마다 한 열로 1M 토큰당 Standard 입력, 출력 단가(`$4.00 / $20.00`)를 보여 준다. 단가마다 번호 각주가 붙고 누르면 참고 자료 목록의 해당 항목으로 이동하며, CSV, Markdown, JSON으로 내려받는다(`/api/pricing/export?format=csv|md|json&lang=ko|en`, 파일 `llm-monitor-unit-prices-YYYY-MM-DD.<ext>`). 상단 호박색 안내 상자와 참고 자료 끝에 "이 가격표는 공개 자료를 자동으로 수집해 정리한 참고용 정보이며, AWS의 공식 입장이 아닙니다. 최종 가격은 반드시 공식 사이트에서 확인하세요."를 표시하고, 모든 다운로드 파일에도 같은 문구를 넣는다. 가장 최근에 끝난 동기화가 확인하지 못한 셀에는 "자동 확인 안 됨", 큰 변경이 승인을 기다리는 셀에는 "검토 대기" 배지가 붙는다. GPT-5.6 Sol에는 수동 메모인 프로모션 배지(최소 2026-11-21까지)가 붙고, 그 날짜가 지나면 "프로모션 종료 여부 확인 필요"로 바뀐다. 배지 설명(마지막 확인일 또는 "초기값", 검토 대기 중인 새 값, 수동 메모 각주가 붙은 프로모션 이전 단가)은 마우스를 올려야 보이는 툴팁이 아니라 배지 옆 글자라 터치, 키보드 사용자도 본다. "모델 ID 보기" 토글을 누르면 단가마다 해당 모델 ID가 보인다. 폰에서는 표만 가로로 스크롤되며, 처음 보이는 단가 열이 Claude Platform on AWS라 OpenAI나 Nova 행이 비어 보이지 않도록 표 위 안내 문구와 오른쪽 가장자리 흐림으로 스크롤할 수 있음을 알린다. 메뉴는 "비용" 바로 뒤다. 표는 활성 55채널의 패밀리 19개를 담고, 휴면 1P와 숨김 채널은 뺀다.
- **Official prices synced every 12 hours.** A new scheduled PricingSync task (`python -m pricing_sync_runner --once`, `rate(12 hours)`) reads three official sources: the Bedrock agreement-offer rate card (`ListFoundationModelAgreementOffers`, 18 foundation models covering the 20 Bedrock Claude and 25 OpenAI channels), the AWS Price List API (`GetProducts`, Nova 2.0 Lite, 1K-token prices × 1000) and Anthropic's `pricing.md` (the 9 Claude Platform on AWS channels at standard pricing). Offer dimensions must fully match an allow-list regex, so batch, flex, priority, long-context, cache, reserved, GovCloud and other region prefixes never count; the Anthropic table is read by header names with exact model-name matching, so "Claude Opus 5.5" never lands on Opus 5. A change of at most 50% on both input and output (50% included) applies from the start of the run that observed it; above that it waits as `pending_review` for an admin (`GET /api/admin/pricing/pending`, `POST /api/admin/pricing/pending/{id}/approve`, `POST …/reject`). A channel whose official value cannot be read keeps its price and shows as not auto-verified; a parser error of any type only skips that source's channels (`skipped:parse_failed`) and never fails the run. Observed prices are compared and stored at 6 decimals, and a value that rounds to 0 is rejected. Runs never overlap: `pg_try_advisory_lock(917350004)` does not wait, so a second run exits at once (exit 1, no run row). A run is capped at 300 s and never stores or logs `offerToken` or the presigned `legalTerm.url`. Every run error (a failed call, a parser error, a model missing from the table, the deadline) is logged as a `pricing sync: …` warning and also kept in the run summary (`price_sync_runs.summary.errors`, first 50).
- **공식 단가를 12시간마다 동기화한다.** 새 스케줄 태스크 PricingSync(`python -m pricing_sync_runner --once`, `rate(12 hours)`)가 공식 출처 3개를 읽는다. Bedrock agreement offer rate card(`ListFoundationModelAgreementOffers`, 파운데이션 모델 18개로 Bedrock Claude 20채널과 OpenAI 25채널), AWS Price List API(`GetProducts`, Nova 2.0 Lite, 1K 토큰 단가 × 1000), Anthropic `pricing.md`(Claude Platform on AWS 9채널, 표준 단가)다. 오퍼 차원 이름은 허용 목록 정규식에 완전히 일치해야 하므로 batch, flex, priority, long-context, cache, reserved, GovCloud, 그 밖의 리전 접두는 후보가 되지 않는다. Anthropic 표는 헤더 이름으로 열을 찾고 모델명을 정확 일치로만 연결하므로 "Claude Opus 5.5"가 Opus 5에 붙지 않는다. 입력과 출력 모두 50% 이하로 바뀌면(50% 포함) 관측한 런의 시작 시각부터 적용하고, 그보다 크면 `pending_review`로 두고 관리자 승인을 기다린다(`GET /api/admin/pricing/pending`, `POST /api/admin/pricing/pending/{id}/approve`, `POST …/reject`). 공식 값을 읽지 못한 채널은 기존 단가를 유지하고 자동 확인 안 됨으로 표시된다. 파서 오류는 종류와 상관없이 그 출처의 채널만 건너뛰고(`skipped:parse_failed`) 런을 실패시키지 않는다. 관측 값은 소수 6자리로 정규화해 비교, 저장하고 0으로 반올림되는 값은 거부한다. 런은 `pg_try_advisory_lock(917350004)`로 겹치지 않게 한다. 기다리지 않는 잠금이라 겹치면 두 번째 런은 즉시 exit 1로 끝난다(런 행 없음). 런에는 300초 상한을 두며, `offerToken`과 presigned `legalTerm.url`은 저장하거나 로그에 남기지 않는다. 런 오류(호출 실패, 파서 오류, 표에 없는 모델, 상한 초과)는 문구마다 `pricing sync: …` 경고 로그로 남기고 런 요약(`price_sync_runs.summary.errors`, 앞 50개)에도 저장한다.
- **`GET /api/pricing`** (public, 60 s in-process cache) returns the families in display order with `tiers` `cp`, `global`, `us` and `in_region` (always an array; regions with the same price are grouped), `models` (model_id → current price), `references` with the footnote numbers assigned by the backend, `last_sync`, `pending_review` (the number of channels waiting for review) and the disclaimer in both languages. The export's JSON is the same body.
- **`GET /api/pricing`**(공개, 60초 인메모리 캐시)는 패밀리를 표시 순서대로 내려주며 `tiers`는 `cp`, `global`, `us`, `in_region`(항상 배열, 같은 단가의 리전은 한 원소로 묶음)이다. `models`(model_id → 현재 단가), 백엔드가 번호를 매긴 `references`, `last_sync`, `pending_review`(검토 대기 채널 수), 두 언어 면책 문구를 함께 싣는다. 다운로드 JSON은 같은 본문이다.
...
- **Model Explorer and Comparison Lab read prices from `/api/pricing` `models`**, formatted like the price table, and "1M in/out" is translated in KO and EN. Comparison Lab shows "—" for a model without a price and leaves it out of the cheapest highlight. The Cost page methodology text now says prices are refreshed from official sources every 12 hours and applied at each probe's time, and the note under the per-model table says costs use those official prices; both link to Unit Prices.
- **모델 탐색과 Comparison Lab은 `/api/pricing` `models`에서 단가를 읽는다.** 표기는 단가 표와 같은 포맷터를 쓰고 "1M in/out"은 KO, EN 모두 번역했다. Comparison Lab은 단가가 없는 모델을 "—"로 표시하고 최저 비용 강조에서 뺀다. 비용 화면 방법론 문단은 "단가는 공식 출처에서 12시간마다 자동 갱신되며 각 프로브 시각의 단가로 계산한다"로, 모델별 표 아래 설명은 공식 출처 단가 기반으로 바꾸고 둘 다 비용 단가 메뉴 링크를 달았다.
...
- `backend/pricing.py`(`PRICE_TABLE`, `get_pricing`, `_normalize_key`, `estimate_cost_usd`)를 삭제했다. `frontend/src/lib/pricing.ts`의 프런트 미러(`PRICE_TABLE`, `getPricing`, `estimateCost`)를 삭제하고 `formatCost`만 남겼다. 호환 모듈은 없다.

### Infra
- CDK `BedrockMonitor-Scheduler`: new `PricingSyncTaskRole` (inline policy `bedrock:ListFoundationModelAgreementOffers` and `pricing:GetProducts` on `*` only, no model invocation), `PricingSyncTaskDef` from the shared `buildTaskDef` (the AutoProber env and secrets for Claude Platform on AWS discovery and OpenAI registration, 0.5 vCPU / 1 GB, log group `/ecs/pricingsync` kept 14 days), `PricingSyncSchedule` `rate(12 hours)`, the new family in the scheduler role's `RunTaskFamilyWildcard` and the new role in `PassTaskRoles` (ADR-011), and the output `PricingSyncScheduleName`. CDK tests pin 6 schedules, 6 task definitions, the command, the two IAM actions and the scheduler grants (85 tests). No new env. The tables `price_history` and `price_sync_runs` come from `create_all`, and backend startup inserts the seed under `pg_advisory_xact_lock(917350003)` after transaction-local `statement_timeout` 30 s and `lock_timeout` 5 s. Deploy with the digest-pinned CDK path (`--exclusively BedrockMonitor-AppServices BedrockMonitor-Scheduler`); the image-only path cannot create the new task definition. After the deploy, run PricingSync once by hand (`docs/runbooks/deploy.md` §5-4).
- CDK `BedrockMonitor-Scheduler`: `PricingSyncTaskRole`(인라인 정책 `bedrock:ListFoundationModelAgreementOffers`, `pricing:GetProducts`, Resource `*`만, 모델 호출 권한 없음), 공용 `buildTaskDef`로 만든 `PricingSyncTaskDef`(Claude Platform on AWS 디스커버리와 OpenAI 등록을 위해 AutoProber와 같은 env와 secret, 0.5 vCPU / 1 GB, 로그 그룹 `/ecs/pricingsync` 14일 보존), `PricingSyncSchedule` `rate(12 hours)`, Scheduler 역할 `RunTaskFamilyWildcard`의 새 family와 `PassTaskRoles`의 새 역할(ADR-011), output `PricingSyncScheduleName`을 추가했다. CDK 테스트가 스케줄 6개, 태스크 정의 6개, 명령, IAM 액션 2개, Scheduler 권한을 고정한다(85건). 신규 env는 없다. `price_history`, `price_sync_runs` 테이블은 `create_all`이 만들고 backend 기동이 트랜잭션 한정 `statement_timeout` 30초, `lock_timeout` 5초를 건 뒤 `pg_advisory_xact_lock(917350003)` 아래 seed를 넣는다. digest 고정 CDK 경로(`--exclusively BedrockMonitor-AppServices BedrockMonitor-Scheduler`)로 배포한다. 이미지-only 경로는 새 태스크 정의를 만들지 못한다. 배포 뒤 PricingSync를 한 번 수동으로 실행한다(`docs/runbooks/deploy.md` §5-4).

### Docs
- New **ADR-030** (official price sources, per-`model_id` price history, the seed and the 11 corrected channels, the 50% guard and admin approval, time-effective costs, no reconstruction before v2.30.0, the manual Sol promotion note, infra). ADR-025 and ADR-027 gain v2.30.0 follow-ups that point to it, and ADR-028 records that the GPT-6 Sol and Luna model cards are now published and match the offers. Updated: `README.md` (EN/KO features, eleven pages, usage, project tree, API table), `docs/architecture.md` (six scheduled tasks in both Mermaid diagrams, the unit price path, tables, ADR 030), `docs/api-reference.md` (`/api/pricing`, `/api/pricing/export`, admin approval, cost semantics), `CLAUDE.md`, `backend/CLAUDE.md`, `backend/routers/CLAUDE.md`, `backend/tests/CLAUDE.md`, `frontend/src/lib/CLAUDE.md`, `frontend/src/components/CLAUDE.md`, `cdk/CLAUDE.md`, and the runbooks (`deploy.md` §5-4 PricingSync manual run and checks; `troubleshooting.md` sync failures, pending approval, the GPT-5.6 Sol promotion check after 2026-11-21; `rollback.md` A-2 and `deploy.md` §5-4 re-deploying v2.30.0 after a CDK rollback, when the retained `/ecs/pricingsync` log group must be deleted or imported with `--import-existing-resources` first; `rollback.md` A-3 disabling the PricingSync schedule, or rolling back through A-2, when the restored image predates v2.30.0).
- **ADR-030**을 신설했다(공식 단가 출처, `model_id` 단위 단가 이력, seed와 교정 11채널, 50% 안전장치와 관리자 승인, 시점 단가 비용, v2.30.0 이전 비재구성, Sol 프로모션 수동 메모, 인프라). ADR-025와 ADR-027에는 이 ADR을 가리키는 v2.30.0 후속 절을, ADR-028에는 GPT-6 Sol, Luna 모델 카드가 게시돼 오퍼 값과 일치한다는 기록을 덧붙였다. `README.md`(두 언어의 기능, 11개 페이지, 사용법, 프로젝트 트리, API 표), `docs/architecture.md`(두 Mermaid 다이어그램의 스케줄 태스크 6개, 단가 경로, 표, ADR 030), `docs/api-reference.md`(`/api/pricing`, `/api/pricing/export`, 관리자 승인, 비용 의미), `CLAUDE.md`, `backend/CLAUDE.md`, `backend/routers/CLAUDE.md`, `backend/tests/CLAUDE.md`, `frontend/src/lib/CLAUDE.md`, `frontend/src/components/CLAUDE.md`, `cdk/CLAUDE.md`, 런북(`deploy.md` §5-4 PricingSync 수동 실행과 확인, `troubleshooting.md` 동기화 실패, 검토 대기 승인, 2026-11-21 이후 GPT-5.6 Sol 프로모션 확인, `rollback.md` A-2와 `deploy.md` §5-4에 CDK 롤백 뒤 v2.30.0 재배포 시 남아 있는 `/ecs/pricingsync` 로그 그룹을 먼저 삭제하거나 `--import-existing-resources`로 가져오는 절차, `rollback.md` A-3에 v2.30.0 이전 image로 되돌릴 때 PricingSync 스케줄을 DISABLED로 바꾸거나 A-2로 되돌리는 절차)을 갱신했다.
...
### Changed
- **Claude Platform on AWS channels are probed every 5-minute cycle again** (user decision 2026-09-26, "Anthropic API 호출 기간 텀을 늘렸었습니다. 원래대로 복귀해 주세요"). This reverts the v2.29.0 10-minute cadence: the 9 CP channels (`anthropic:<id>`, labels `Anthropic Claude … (US)`) run in every cycle with the cycle's workload category, like every other model, as they did before v2.29.0. CP calls double from 54 to 108 probes per hour. If the organization's monthly usage cap returns, its 429 is still not retried (one request and one error row per probe; `prober._is_usage_cap_error` and the `max_retries=0` `_get_anthropic_probe_client` are unchanged).
- **Claude Platform on AWS 채널을 다시 5분 사이클마다 프로빙한다**(2026-09-26 사용자 결정, "Anthropic API 호출 기간 텀을 늘렸었습니다. 원래대로 복귀해 주세요"). v2.29.0의 10분 주기를 되돌린 것이다. CP 9채널(`anthropic:<id>`, 라벨 `Anthropic Claude … (US)`)은 v2.29.0 이전처럼 매 사이클 다른 모델과 같은 워크로드 카테고리로 호출된다. CP 호출은 시간당 54회에서 108회로 두 배가 된다. 조직 월간 사용량 상한이 다시 걸려도 그 429는 여전히 재시도하지 않는다(프로브당 요청 1회, 오류 행 1개. `prober._is_usage_cap_error`와 `max_retries=0`인 `_get_anthropic_probe_client`는 그대로다).
- **`ANTHROPIC_CP_PROBE_INTERVAL_S` stays as an operational knob; its default is now 300.** The code default and the CDK value go from 600 to 300. At 300 or below, `auto_prober._plan_cycle` gives every model the cycle's preset and skips the CP history lookup (`_cp_history`), so CP joins the cycle's category from the first cycle after the deploy. Setting 600 again restores the v2.29.0 behavior (CP every other cycle, decided from the run start time, with CP's own category rotation) as a lever if the usage cap returns; put the same value on the backend service so `/status` matches. The value is now rounded to whole 5-minute cycles when it is read, with ties going down, which matches the due check (interval − 150 s): 301–450 behaves exactly like 300, 451–750 like 600, and `/status` and `/latest` use the rounded value. Before, a value such as 400 was probed every cycle but kept CP's own category rotation.
- **`ANTHROPIC_CP_PROBE_INTERVAL_S`는 운영 노브로 남기고 기본값을 300으로 바꿨다.** 코드 기본값과 CDK 주입값이 600에서 300이 됐다. 300 이하에서는 `auto_prober._plan_cycle`이 모든 모델에 사이클 preset을 주고 CP 이력 조회(`_cp_history`)를 건너뛰므로, 배포 뒤 첫 사이클부터 CP가 사이클 카테고리에 합류한다. 다시 600으로 올리면 v2.29.0 동작(run 시작 시각 기준 두 사이클에 한 번, CP 자체 카테고리 회전)이 돌아온다. 사용량 상한이 다시 걸릴 때 쓰는 레버이며, `/status`가 맞도록 backend 서비스에도 같은 값을 넣는다. 값은 이제 읽을 때 5분 사이클의 배수로 반올림하고 동률은 내린다. due 판정(주기 − 150초)과 같은 결과로, 301~450은 300과, 451~750은 600과 똑같이 동작하며 `/status`와 `/latest`도 반올림한 값을 쓴다. 이전에는 400 같은 값이면 매 사이클 프로빙되면서도 CP 자체 카테고리 회전이 남았다.
- **The API contract is unchanged; only the values follow the new default.** `/api/auto-probe/status` keeps `channel_intervals` and `channel_category_intervals`, which now read `{"anthropic": 300}` and `{"anthropic": 1800}` (the same as `interval_seconds` and `category_interval_seconds`). `/api/auto-probe/latest` still returns each model's latest row; the per-model windows are sized by `probe_cadence.interval_for`, so the CP windows shrink to the base 15 minutes (60 minutes with `?category=`). The chatbot `get_latest_results` tool shares the helper.
- **API 계약은 그대로이고 값만 새 기본값을 따른다.** `/api/auto-probe/status`는 `channel_intervals`와 `channel_category_intervals`를 그대로 내보내며, 값은 `{"anthropic": 300}`, `{"anthropic": 1800}`이 된다(`interval_seconds`, `category_interval_seconds`와 같다). `/api/auto-probe/latest`는 여전히 모델별 최신 행을 돌려준다. 모델별 범위는 `probe_cadence.interval_for`로 정하므로 CP 범위도 기본 15분(`?category=`는 60분)으로 줄어든다. 챗봇 `get_latest_results` 도구도 같은 헬퍼를 쓴다.
- **The dashboard no longer assumes 10 minutes for CP before `/status` loads.** Until `/status` arrives, or when it fails, every channel uses the base cadence for freshness and trend gaps; after that `channel_intervals` decides, and the "Claude Platform on AWS every 10 min" note stays hidden because its value equals the base cadence. The Reliability page explanation drops its CP 10-minute sentence (KO/EN). With the 600 knob, a CP card whose last row is older than 10 minutes can therefore show "stale" briefly until `/status` loads (until a refresh succeeds if the first `/status` request fails); `docs/runbooks/deploy.md` §5-2 notes this.
- **대시보드는 `/status`를 불러오기 전에 CP 10분 주기를 가정하지 않는다.** `/status`가 오기 전이나 실패했을 때는 모든 채널의 신선도와 추세 끊김을 기본 주기로 판정하고, 그 뒤에는 `channel_intervals`를 따른다. 값이 기본 주기와 같으므로 "Claude Platform on AWS 10분 주기" 안내는 보이지 않는다. 신뢰성 화면 설명에서도 CP 10분 주기 문장을 뺐다(KO/EN). 그래서 600 노브를 쓰면 마지막 행이 10분을 넘긴 CP 카드가 `/status`를 불러오기 전까지(첫 `/status` 요청이 실패하면 새로고침이 성공할 때까지) 잠시 "수집 지연"으로 보일 수 있다. `docs/runbooks/deploy.md` §5-2에 적어 두었다.

### Fixed
- **SQLAlchemy is pinned below 2.1** (`backend/requirements.txt`: `sqlalchemy>=2.0.0,<2.1`). SQLAlchemy 2.1.1 reached CI on 2026-09-26 and makes psycopg (v3) the default driver for plain `postgresql://` URLs, while the project only installs `psycopg2-binary`, so every test module that imports the database failed with `ModuleNotFoundError: No module named 'psycopg'`. The next `--no-cache --pull` image build would have shipped the same break to the backend and all five scheduled tasks. The production image runs 2.0.54, which the pin keeps; with it, 412 backend tests pass in a fresh environment. Moving to 2.1 later means adding `psycopg[binary]` or writing `postgresql+psycopg2://` in `database.py`.
- **SQLAlchemy를 2.1 미만으로 고정했다**(`backend/requirements.txt`: `sqlalchemy>=2.0.0,<2.1`). 2026-09-26 CI에 SQLAlchemy 2.1.1이 설치됐는데, 2.1은 일반 `postgresql://` URL의 기본 드라이버를 psycopg(v3)로 바꾼다. 이 프로젝트는 `psycopg2-binary`만 설치하므로 DB를 import하는 테스트 모듈이 모두 `ModuleNotFoundError: No module named 'psycopg'`로 실패했다. 다음 `--no-cache --pull` 이미지 빌드였다면 같은 오류가 backend와 스케줄 태스크 5개 전체에 배포됐을 것이다. 운영 이미지는 2.0.54이고 이 고정으로 같은 버전이 유지되며, 새 환경에서 backend 테스트 412개가 통과한다. 나중에 2.1로 올리려면 `psycopg[binary]`를 추가하거나 `database.py`에서 `postgresql+psycopg2://`를 쓰면 된다.

### Infra
- CDK `BedrockMonitor-Scheduler`: the AutoProber task definition sets `ANTHROPIC_CP_PROBE_INTERVAL_S=300` (was 600). No IAM, DB or schedule change; FeaturesVerify stays `cron(30 17 * * ? *)` Etc/UTC. Deploy with the digest-pinned CDK path (`BedrockMonitor-AppServices` + `BedrockMonitor-Scheduler`, backend and frontend images), not the image-only path, which would copy the old `600` into the new task definition and keep CP at every other cycle. Verification steps are in `docs/runbooks/deploy.md` §5-3.
- CDK `BedrockMonitor-Scheduler`: AutoProber 태스크 정의가 `ANTHROPIC_CP_PROBE_INTERVAL_S=300`을 명시한다(이전 600). IAM, DB, 스케줄 변경은 없고 FeaturesVerify는 `cron(30 17 * * ? *)` Etc/UTC 그대로다. 이미지-only 경로는 기존 `600`을 새 태스크 정의에 복사해 CP를 계속 두 사이클에 한 번으로 남기므로, digest 고정 CDK 경로(`BedrockMonitor-AppServices` + `BedrockMonitor-Scheduler`, backend와 frontend 이미지)로 배포한다. 확인 절차는 `docs/runbooks/deploy.md` §5-3에 있다.

### Docs
- **README screenshots from production** (2026-09-26, v2.29.0, dark theme, 1440x900): the Overview hero now shows the current dashboard, and a new Screenshots gallery covers graded model cards, trend charts, GPT on AWS, Claude API Features, Parity Run, Cost and Reliability. English captures go in the English section and Korean captures in the Korean section (`docs/images/ui/*-en.png`, `*-ko.png`, 16 files, 1.9 MB). The v1-era `docs/images/dashboard.png` is removed. The folder is `ui/` because `.gitignore` ignores every directory named `screenshots/`.
- **운영 화면 스크린샷을 README에 넣었다**(2026-09-26, v2.29.0, 다크 테마, 1440x900). 개요 대표 이미지를 현재 대시보드로 바꿨고, 새 스크린샷 갤러리에 지표 등급 모델 카드, 추이 차트, GPT on AWS, Claude API 기능, 패리티 런, 비용, 신뢰성 화면을 담았다. 영문 섹션에는 영문 UI, 한국어 섹션에는 한국어 UI 캡처를 쓴다(`docs/images/ui/*-en.png`, `*-ko.png`, 16개, 1.9 MB). v1 시절 `docs/images/dashboard.png`는 지웠다. `.gitignore`가 `screenshots/`라는 이름의 디렉터리를 모두 무시하므로 폴더 이름은 `ui/`다.
- **`docs/architecture.md` rewritten** in the project's bilingual layout (English first, full sections in both languages) with Mermaid diagrams (layered `flowchart TB` + two `flowchart LR` data paths, rendered with Mermaid 11) replacing the ASCII boxes. Facts corrected against CDK and the live account: no WAF is attached (`edge-stack.ts` defers the CLOUDFRONT-scope ACL to a separate us-east-1 stack); CloudFront serves the CDK-owned alias `llm-monitor.whchoi.net`; the VPC Origin reaches the internal ALB over HTTP port 80 inside the VPC (temporary `HTTP_ONLY`); `bedrock-monitor-backend-v2` is IMMUTABLE but created outside CDK while the CDK repos are MUTABLE; RDS is PostgreSQL 16.8; the CloudWatch dashboard has 4 graph widgets plus an alarm status widget. Adds cache behaviors, SSM parameters, SES, OptimizePrompt, the scheduled-task table and the security table.
- **`docs/architecture.md`를 다시 썼다.** 프로젝트의 이중 언어 형식(영문 먼저, 두 언어 모두 전체 섹션)에 맞추고, ASCII 상자 대신 Mermaid 다이어그램(레이어별 `flowchart TB`와 데이터 경로 `flowchart LR` 2개, Mermaid 11로 렌더링 확인)을 넣었다. CDK와 운영 계정을 대조해 사실을 바로잡았다. WAF는 연결돼 있지 않다(`edge-stack.ts`가 CLOUDFRONT scope ACL을 별도 us-east-1 스택으로 미룸). CloudFront는 CDK가 소유한 alias `llm-monitor.whchoi.net`을 쓴다. VPC Origin은 VPC 내부에서 HTTP 포트 80으로 내부 ALB에 연결한다(임시 `HTTP_ONLY`). `bedrock-monitor-backend-v2`는 IMMUTABLE이지만 CDK 밖에서 만든 저장소이고 CDK 저장소는 MUTABLE이다. RDS는 PostgreSQL 16.8이다. CloudWatch 대시보드는 그래프 위젯 4개와 알람 상태 위젯 1개다. 캐시 behavior, SSM 파라미터, SES, OptimizePrompt, 스케줄 태스크 표, 보안 표도 추가했다.
- **CLAUDE.md sync.** Root: insights run on Sonnet 4.6 (Haiku 4.5 only writes chat follow-ups), `src/proxy.ts` instead of `middleware.ts`, GptBench in the scheduling line, `/chat` and `/?view=manual` pages, missing backend, frontend and CDK files in the tree, a Test command block, digest pinning attributed to `cdk/lib/constructs/pinned-image.ts` (not ADR-018), `EXCLUDED_FAMILIES` in `lib/sortModels.ts`, complete public and auth-required endpoint lists, accurate ECR mutability, `lock_timeout` and the 300 s health-check grace, 10 more environment variables, and the `.kiro/specs/v2-upgrade/` link marked as the v2.0.0 design record. The `from __future__ import annotations` rule now matches the code (allowed, with signature types at module level). Five module files updated and eight new ones added: `backend/agent/`, `backend/claude_features/`, `backend/tests/`, `frontend/src/app/`, `frontend/src/hooks/`, `frontend/src/lib/`, `frontend/src/components/chat/`, `cdk/`.
- **CLAUDE.md를 코드와 맞췄다.** 루트 파일은 다음을 반영했다. 인사이트는 Sonnet 4.6이고 Haiku 4.5는 챗 후속 질문만 만든다. `middleware.ts` 대신 `src/proxy.ts`다. 스케줄 줄에 GptBench를 넣었다. `/chat`, `/?view=manual` 페이지를 추가했다. 트리에 빠진 backend, frontend, CDK 파일을 넣었다. 테스트 명령 블록을 만들었다. digest 고정의 출처를 ADR-018이 아닌 `cdk/lib/constructs/pinned-image.ts`로 고쳤다. 제외 필터 위치를 `lib/sortModels.ts` `EXCLUDED_FAMILIES`로 바로잡았다. 공개, 인증 필요 엔드포인트 목록을 채웠다. ECR 가변성을 사실대로 적었다. `lock_timeout`과 헬스체크 유예 300초를 넣었다. 환경 변수 10개를 추가했다. `.kiro/specs/v2-upgrade/` 링크를 v2.0.0 설계 기록으로 표시했다. `from __future__ import annotations` 규칙은 코드에 맞게 바꿨다(허용, 시그니처 타입은 모듈 전역 이름). 모듈 파일 5개를 고쳤고 `backend/agent/`, `backend/claude_features/`, `backend/tests/`, `frontend/src/app/`, `frontend/src/hooks/`, `frontend/src/lib/`, `frontend/src/components/chat/`, `cdk/`에 새 파일 8개를 만들었다.
- **Runbooks and API reference.** `deploy.md`: bootstrap in ap-northeast-2, one-time `bedrock-monitor-backend-v2` creation, digest capture, no CLI wildcards, schedule name from the `AutoProberScheduleName` output, ALB certificate in ap-northeast-2, digest-pinned `--exclusively` deploys, `make build` and `--force-new-deployment` marked as not shipping new code. `rollback.md`: ap-northeast-2, rollback by redeploying the previous tag@digest (primary) or an ACTIVE task definition revision, `continue-update-rollback` only for UPDATE_ROLLBACK_FAILED, a recording loop for 2 services and 5 scheduled tasks, old v2.6.2/v2.7.0 points moved to history. `api-reference.md`: `/api/health`, `/api/auto-probe/categories`, `GET /api/probes/{run_id}`, `POST /api/prompts/optimize`, `POST /api/insights/regenerate`, real `/api/probes/run` limits and `concurrency`, `/api/results` `run_id`/`start_time`/`end_time`.
- **런북과 API 레퍼런스를 고쳤다.** `deploy.md`는 ap-northeast-2 bootstrap, `bedrock-monitor-backend-v2` 최초 1회 생성, digest 확인, CLI 와일드카드 제거, `AutoProberScheduleName` 출력으로 스케줄 이름 조회, ap-northeast-2 ALB 인증서, digest 고정 `--exclusively` 배포를 반영했고, `make build`와 `--force-new-deployment`는 새 코드를 내보내지 않는다고 표시했다. `rollback.md`는 ap-northeast-2, 직전 tag@digest 재배포(주 경로) 또는 ACTIVE 태스크 정의 revision으로 되돌리기, `continue-update-rollback`은 UPDATE_ROLLBACK_FAILED에서만, 서비스 2개와 스케줄 태스크 5개 기록 루프를 넣었고 v2.6.2/v2.7.0 지점은 기록 보관으로 옮겼다. `api-reference.md`에는 `/api/health`, `/api/auto-probe/categories`, `GET /api/probes/{run_id}`, `POST /api/prompts/optimize`, `POST /api/insights/regenerate`, `/api/probes/run`의 실제 한도와 `concurrency`, `/api/results`의 `run_id`/`start_time`/`end_time`을 추가했다.
- **README sync.** Taglines and Overview include OpenAI GPT on Bedrock (21 Bedrock + 9 Claude Platform on AWS + 25 OpenAI = 55); installation adds the `openai-api-key` SSM parameter, the one-time `bedrock-monitor-backend-v2` repository, the Cluster stack before the first push, optional RUM build args and digest-pinned deploys; the configuration table, project tree, testing commands and API table are filled in; the CI description matches `.github/workflows/ci.yml`; English and Korean sections carry the same facts.
- **README를 맞췄다.** 태그라인과 개요에 OpenAI GPT on Bedrock을 넣었다(Bedrock 21 + Claude Platform on AWS 9 + OpenAI 25 = 55). 설치 절차에 `openai-api-key` SSM 파라미터, `bedrock-monitor-backend-v2` 저장소 최초 1회 생성, 첫 push 전 Cluster 스택 배포, 선택 RUM build arg, digest 고정 배포를 넣었다. 환경 설정 표, 프로젝트 트리, 테스트 명령, API 표를 채웠고, CI 설명을 `.github/workflows/ci.yml`에 맞췄으며, 영문과 한국어 섹션의 사실을 일치시켰다.
- **v2.29.1 cadence revert in the docs.** `CLAUDE.md` (scheduling line, architecture block, tree, workload note, AutoProber notes, `ANTHROPIC_CP_PROBE_INTERVAL_S` row), `backend/CLAUDE.md`, `backend/agent/CLAUDE.md`, `backend/routers/CLAUDE.md`, `backend/tests/CLAUDE.md`, `cdk/CLAUDE.md`, `README.md` (features, configuration table and screenshot caption in both languages), `docs/architecture.md` (KO/EN, both Mermaid diagrams), `docs/api-reference.md` (`/status` example and `/latest` windows), `docs/runbooks/deploy.md` (new §5-3; §5-2's CP checks now describe the 600 mode) and `docs/runbooks/troubleshooting.md` (usage-cap expectations and the 600 lever). Tests: `backend/tests/test_cp_cadence.py`, `test_auto_probe_latest.py` (430 backend tests), `frontend/src/lib/monitoring.test.ts`, `pivotTrend.test.ts`, `frontend/e2e/monitoring.spec.ts`, `cdk/test/scheduler-stack.test.ts`. Version **v2.29.1**.
- **v2.29.1 주기 복귀를 문서에 반영했다.** `CLAUDE.md`(스케줄 줄, 아키텍처 블록, 트리, 워크로드 안내, AutoProber 메모, `ANTHROPIC_CP_PROBE_INTERVAL_S` 행), `backend/CLAUDE.md`, `backend/agent/CLAUDE.md`, `backend/routers/CLAUDE.md`, `backend/tests/CLAUDE.md`, `cdk/CLAUDE.md`, `README.md`(두 언어의 기능, 환경 설정 표, 스크린샷 설명), `docs/architecture.md`(KO/EN, Mermaid 다이어그램 2곳), `docs/api-reference.md`(`/status` 예시, `/latest` 범위), `docs/runbooks/deploy.md`(§5-3 신설, §5-2의 CP 확인은 600 모드 기준으로 표시), `docs/runbooks/troubleshooting.md`(사용량 상한 기댓값과 600 레버)를 고쳤다. 테스트는 `backend/tests/test_cp_cadence.py`, `test_auto_probe_latest.py`(backend 테스트 430개), `frontend/src/lib/monitoring.test.ts`, `pivotTrend.test.ts`, `frontend/e2e/monitoring.spec.ts`, `cdk/test/scheduler-stack.test.ts`다. 버전을 **v2.29.1**로 올렸다.

## v2.29.0 — 2026-09-24

### Changed
- **Claude Platform on AWS channels are probed every 10 minutes** (user request 2026-09-23, "API throttling, 1P only"). Only the 9 CP channels (`anthropic:<id>`, labels `Anthropic Claude … (US)`, the Anthropic first-party API) move off the 5-minute cadence; Bedrock Claude (`global.`/`us.` profiles), Nova and OpenAI stay at 5 minutes and the EventBridge schedule is still `rate(5 minutes)`. Reason: since 2026-09-23 19:52 UTC every CP call returns the organization's monthly usage-cap 429 (see Fixed), and the slower cadence halves CP calls from 108 to 54 probes per hour. `auto_prober._plan_cycle` decides per channel from the DB: a CP channel is due when the run holding its latest automatic row started at least `ANTHROPIC_CP_PROBE_INTERVAL_S` (default 600) − 150 s before the current run, or when it has no row in the last hour. Run start times are used instead of row timestamps because CP rows land 1–2 minutes into a cycle, and the half-cycle tolerance absorbs scheduler start jitter (runs 281–312 s apart were observed).
- **Claude Platform on AWS 채널은 10분마다 프로빙한다**(2026-09-23 사용자 요청, "API 스로틀링 이슈, 1P만 해당"). 5분 주기에서 빠지는 것은 CP 9채널(`anthropic:<id>`, 라벨 `Anthropic Claude … (US)`, Anthropic 1P API)뿐이다. Bedrock Claude(`global.`, `us.` 프로파일), Nova, OpenAI는 5분 그대로이고 EventBridge 스케줄도 여전히 `rate(5 minutes)`다. 이유는 2026-09-23 19:52 UTC부터 CP 호출이 전부 조직 월간 사용량 상한 429를 받기 때문이며(아래 Fixed), 주기를 늘려 CP 호출을 시간당 108회에서 54회로 절반으로 줄인다. `auto_prober._plan_cycle`이 채널마다 DB로 판정한다. 직전 자동 행이 속한 run이 이번 run보다 `ANTHROPIC_CP_PROBE_INTERVAL_S`(기본 600) − 150초 이상 먼저 시작했거나 최근 1시간에 행이 없으면 프로빙한다. 결과 행 timestamp 대신 run 시작 시각을 쓰는 것은 CP 행이 사이클 시작 1~2분 뒤에 찍히기 때문이고, 사이클 절반의 허용 오차는 스케줄 기동 지연(실측 run 간격 281~312초)을 흡수한다.
- **CP channels rotate the six workload categories on their own.** Each CP channel takes the category after its own last one (the cycle's category for a new channel or after a long gap), so every category still comes round about once an hour and a CP row can carry a different category, prompt and `max_tokens` than the other models in the same run. The round-robin for every other model now reads the latest non-CP automatic row, so it picks the same category as before.
- **CP 채널은 6개 워크로드 카테고리를 따로 순환한다.** 채널마다 자기 직전 카테고리의 다음 것을 쓰고(새 채널이나 긴 공백 뒤에는 사이클 카테고리), 그래서 카테고리마다 약 60분에 한 번씩 돌아오며, 같은 run 안에서도 CP 행의 카테고리, 프롬프트, `max_tokens`가 다른 모델과 다를 수 있다. 나머지 모델의 라운드로빈은 CP가 아닌 최신 자동 행을 읽으므로 이전과 같은 카테고리를 고른다.
- **`/api/auto-probe/latest` returns each model's latest row, not the rows of one run.** The window is anchored at the start of the latest completed automatic run and sized by the model's own cadence (3 intervals: 15 min, CP 30 min; with `?category=` 2 rotations: 60 min, CP 120 min), so CP cards keep their last result on the cycles that skip them and the workload view shows CP too. Two bounded queries after the anchor (a `probe_results.timestamp` range aggregate, then a primary-key fetch); the response shape is unchanged. The chatbot `get_latest_results` tool uses the same helper and now includes each row's timestamp. `/api/auto-probe/status` adds `channel_intervals` (`{"anthropic": 600}`) and `channel_category_intervals` (`{"anthropic": 3600}`); existing fields are unchanged.
- **`/api/auto-probe/latest`는 run 하나의 행이 아니라 모델별 최신 행을 돌려준다.** 범위는 최신 완료 자동 run의 시작 시각을 기준으로 모델 자기 주기에 맞춘다(주기 3회: 15분, CP 30분. `?category=`는 회전 2바퀴: 60분, CP 120분). 그래서 CP를 건너뛴 사이클에도 CP 카드가 마지막 결과를 유지하고 워크로드 보기에도 CP가 나온다. 기준 run 조회 뒤 bounded 쿼리 2개(`probe_results.timestamp` 범위 집계, PK 조회)이며 응답 형태는 그대로다. 챗봇 `get_latest_results` 도구도 같은 헬퍼를 쓰고 행별 timestamp를 함께 준다. `/api/auto-probe/status`에 `channel_intervals`(`{"anthropic": 600}`)와 `channel_category_intervals`(`{"anthropic": 3600}`)를 추가했다. 기존 필드는 그대로다.
- **The dashboard judges freshness and trend gaps per channel.** CP cards stay fresh for 10 minutes plus 5 minutes of grace (60 minutes plus grace in a workload view) instead of turning stale every other cycle, and CP trend lines no longer break between 10-minute points; other channels are unchanged. The collection line shows "Claude Platform on AWS 10분 주기" next to the 5-minute cadence, and the workload note adds the CP cadence (KO/EN). Without the new status fields everything falls back to the base cadence.
- **대시보드가 신선도와 추세 끊김을 채널별로 판정한다.** CP 카드는 두 사이클에 한 번씩 "수집 지연"으로 바뀌지 않고 10분 + 5분 유예(워크로드 보기는 60분 + 유예) 동안 정상으로 남으며, CP 추세 선은 10분 간격 점 사이에서 끊기지 않는다. 다른 채널은 그대로다. 수집 상태 줄에 5분 주기와 함께 "Claude Platform on AWS 10분 주기"가 보이고, 워크로드 안내에도 CP 주기가 붙는다(KO/EN). 새 status 필드가 없으면 모두 기본 주기로 돌아간다.
- **FeaturesVerify runs once a day at a fixed 17:30 UTC** (02:30 KST, user request 2026-09-23 "one turn per day"). The schedule moves from `rate(24 hours)` to `cron(30 17 * * ? *)` with timezone `Etc/UTC`. The rate schedule already ran once a day (about 17:28–17:38 UTC, from its creation time); on 2026-09-23 a manual run at 09:14 made it look like two turns.
- **FeaturesVerify는 매일 17:30 UTC(02:30 KST) 고정으로 한 번 돈다**(2026-09-23 사용자 요청 "1일 한번 턴"). 스케줄을 `rate(24 hours)`에서 timezone `Etc/UTC`의 `cron(30 17 * * ? *)`로 바꿨다. rate 스케줄도 하루 한 번이었지만(생성 시각 기준 약 17:28~17:38 UTC), 2026-09-23에는 09:14 수동 실행이 겹쳐 하루 두 번처럼 보였다.

### Fixed
- **The monthly usage-cap 429 is no longer retried.** CP answers `429 rate_limit_error` "You have reached your API usage limits: your organization has crossed its monthly API usage threshold … You will regain access on 2026-10-01 at 00:00 UTC." (`error_code` `enforced_spend_limit_reached`). Its type matched the transient patterns, so the prober loop tried 4 times and the anthropic SDK retried each attempt twice: up to 12 requests per probe and about 1,600–1,700 `/ecs/autoprober` lines per hour. `prober._is_usage_cap_error` ("usage limits", "usage threshold", `enforced_spend_limit_reached`, case-insensitive) now ends the probe after one request with one error row (same `Unexpected: Error code: 429 - …` format) and one warning line without a traceback, on any path.
- **월간 사용량 상한 429를 더 이상 재시도하지 않는다.** CP는 `429 rate_limit_error` "You have reached your API usage limits: your organization has crossed its monthly API usage threshold … You will regain access on 2026-10-01 at 00:00 UTC."(`error_code` `enforced_spend_limit_reached`)를 돌려준다. 타입이 일시 오류 패턴과 같아 prober 루프가 4회 시도했고 anthropic SDK가 시도마다 2회씩 더 재시도해, 프로브 하나가 최대 12요청이 됐고 `/ecs/autoprober`에 시간당 약 1,600~1,700줄이 쌓였다. 이제 `prober._is_usage_cap_error`("usage limits", "usage threshold", `enforced_spend_limit_reached`, 대소문자 무시)가 어느 경로든 요청 1회 뒤 오류 행 1개(형식은 그대로 `Unexpected: Error code: 429 - …`)와 traceback 없는 경고 한 줄로 끝낸다.
- **The CP probe client has no SDK retries.** The dashboard cycle and `/api/probes/run` use `_get_anthropic_probe_client` (`max_retries=0`); the prober loop retries what the SDK used to (HTTP 408/409/429/5xx, connection errors and timeouts) with the existing 2/4/8 s backoff inside the wall-clock budget, so ordinary rate limits and 529 overloaded are still retried. Comparison Lab and the parity run keep the SDK-default `_get_anthropic_client` (they have no retry loop). Non-CP retry behavior is unchanged.
- **CP 프로브 클라이언트에서 SDK 재시도를 뺐다.** 대시보드 사이클과 `/api/probes/run`은 `_get_anthropic_probe_client`(`max_retries=0`)를 쓴다. SDK가 하던 재시도(HTTP 408/409/429/5xx, 연결 오류와 timeout)는 prober 루프가 기존 2/4/8초 backoff로 wall-clock 예산 안에서 맡으므로, 일반 rate limit과 529 overloaded는 계속 재시도된다. Comparison Lab과 패리티 런은 재시도 루프가 없어 SDK 기본값 `_get_anthropic_client`를 그대로 쓴다. CP가 아닌 경로의 재시도 동작은 바뀌지 않았다.

### Infra
- CDK `BedrockMonitor-Scheduler`: `FeaturesVerifySchedule` is `cron(30 17 * * ? *)` Etc/UTC (description "… daily at 17:30 UTC"), and the AutoProber task definition sets `ANTHROPIC_CP_PROBE_INTERVAL_S=600` explicitly (code default is also 600; below 300 means every cycle). No IAM or DB change. Deploy with the digest-pinned CDK path (`BedrockMonitor-AppServices` + `BedrockMonitor-Scheduler`), not the image-only path, so the new env is applied; verification steps are in `docs/runbooks/deploy.md` §5-2 (CP rows appear every other cycle).
- CDK `BedrockMonitor-Scheduler`: `FeaturesVerifySchedule`은 `cron(30 17 * * ? *)` Etc/UTC(description "… daily at 17:30 UTC")이고, AutoProber 태스크 정의가 `ANTHROPIC_CP_PROBE_INTERVAL_S=600`을 명시한다(코드 기본값도 600, 300 미만은 매 사이클). IAM, DB 변경은 없다. 새 env가 반영되도록 이미지-only 경로가 아닌 digest 고정 CDK 경로(`BedrockMonitor-AppServices` + `BedrockMonitor-Scheduler`)로 배포한다. 확인 절차는 `docs/runbooks/deploy.md` §5-2에 있다(CP 행은 두 사이클에 한 번).
- **Operational note**: CP returns the usage-cap 429 until 2026-10-01 00:00 UTC, so the 9 CP cards show errors until then, or until the organization's API tier or usage limit is raised in the Anthropic Console. Nothing needs to change in the monitor; the first CP cycle after the reset recovers on its own. With the 10-minute cadence and no retries, the cap costs about 54 requests per hour instead of up to about 1,300. See `docs/runbooks/troubleshooting.md` for the signature and checks.
- **운영 참고**: CP는 2026-10-01 00:00 UTC까지 사용량 상한 429를 돌려주므로, 그때까지(또는 Anthropic Console에서 조직 API 등급이나 사용량 한도를 올릴 때까지) CP 카드 9장은 오류로 보인다. 모니터 쪽에서 바꿀 것은 없고 상한이 풀린 뒤 첫 CP 사이클에 저절로 돌아온다. 10분 주기와 재시도 제거로 상한 기간 요청은 시간당 최대 약 1,300회에서 약 54회로 줄어든다. 서명과 확인 방법은 `docs/runbooks/troubleshooting.md`에 있다.

### Docs
- `docs/api-reference.md` (`/status` `channel_intervals`, per-model `/latest`), `docs/runbooks/deploy.md` §5-2, `docs/runbooks/troubleshooting.md` (CP usage-cap 429), `docs/architecture.md` (KO/EN), `CLAUDE.md` (architecture, AutoProber notes, `ANTHROPIC_CP_PROBE_INTERVAL_S`), `backend/CLAUDE.md`, `README.md`. Tests: `backend/tests/test_cp_cadence.py`, `test_auto_probe_latest.py`, `test_cp_usage_cap.py` (real anthropic SDK over `httpx.MockTransport`), `frontend/src/lib/monitoring.test.ts`, `pivotTrend.test.ts`, `frontend/e2e/monitoring.spec.ts`, `cdk/test/scheduler-stack.test.ts`. Version **v2.29.0**.
- `docs/api-reference.md`(`/status` `channel_intervals`, 모델별 `/latest`), `docs/runbooks/deploy.md` §5-2, `docs/runbooks/troubleshooting.md`(CP 사용량 상한 429), `docs/architecture.md`(KO/EN), `CLAUDE.md`(아키텍처, AutoProber 메모, `ANTHROPIC_CP_PROBE_INTERVAL_S`), `backend/CLAUDE.md`, `README.md`를 갱신했다. 테스트는 `backend/tests/test_cp_cadence.py`, `test_auto_probe_latest.py`, `test_cp_usage_cap.py`(실제 anthropic SDK + `httpx.MockTransport`), `frontend/src/lib/monitoring.test.ts`, `pivotTrend.test.ts`, `frontend/e2e/monitoring.spec.ts`, `cdk/test/scheduler-stack.test.ts`다. 버전을 **v2.29.0**으로 올렸다.

## v2.28.2 — 2026-09-23

### Fixed
- **The dashboard AutoProber no longer freezes when one model's stream stalls** (production incident 2026-09-23: no completed cycle after 10:05 UTC). Root cause chain: (1) Bedrock Mantle us-east-1 GPT-5.6 Sol (`openai:us-east-1:openai.gpt-5.6-sol`) answered 200 and then its Responses SSE stream stalled or trickled, so no per-read timeout ever fired (a live repro hung 150 s even with an httpx read timeout of 40 s); (2) the prober built its OpenAI client with the SDK defaults (timeout 600 s, `max_retries` 2); (3) `_run_reserved_cycle` waited with `future.result(timeout=120)`, and on timeout marked the **whole run** failed ("1 model probes did not finish normally"), after which leaving `with ThreadPoolExecutor` joined the hung thread, so the Fargate task stayed RUNNING for 30–46 min; (4) that run's 900 s running reservation made the next scheduled tasks log "skipping overlapping cycle"; (5) `/api/auto-probe/latest` serves the last **completed** run, so the dashboard froze even though runs 34046–34048 had written 54 of 55 success rows.
- **대시보드 AutoProber가 모델 하나의 스트림 정지로 멈추지 않는다**(2026-09-23 운영 장애, 10:05 UTC 이후 완료된 사이클 없음). 원인 사슬은 이렇다. 1. Bedrock Mantle us-east-1 GPT-5.6 Sol(`openai:us-east-1:openai.gpt-5.6-sol`)이 200을 준 뒤 Responses SSE 스트림이 멈추거나 드문드문 흘러 청크 간 read timeout이 끝내 발동하지 않았다(라이브 재현: httpx read timeout 40초로도 150초 정지). 2. prober의 OpenAI 클라이언트가 SDK 기본값(timeout 600초, `max_retries` 2)이었다. 3. `_run_reserved_cycle`은 `future.result(timeout=120)`으로 기다리다 타임아웃이 나면 **run 전체**를 failed로 만들었고("1 model probes did not finish normally"), 이어서 `with ThreadPoolExecutor` 종료가 멈춘 스레드를 join해 Fargate 태스크가 30~46분 RUNNING으로 남았다. 4. 그 run의 900초 running 예약 때문에 다음 스케줄 태스크가 "skipping overlapping cycle"로 빠졌다. 5. `/api/auto-probe/latest`는 마지막 **완료** run을 보여 주므로, run 34046~34048이 55개 중 54개 성공 행을 썼는데도 대시보드가 동결됐다.
- **Probe wall-clock watchdog.** Every probe stream (OpenAI Responses, Anthropic CP `messages.stream`, Bedrock `converse_stream`, and the Comparison Lab) now runs under a wall-clock cap, `PROBE_WALL_CLOCK_S` (env, default 90 s, covering retries and backoff): all three SDKs only bound the wait between chunks. The v2.28.0 bench watchdog moved unchanged to the shared `backend/stream_watchdog.py` (`gptbench` re-exports the old names) and can now also shut down the socket under a botocore `EventStream`. On expiry the stream socket is shut down and the model gets an error row `WallClockTimeout: probe exceeded 90s wall-clock` for that cycle, with no retry; a stream whose terminal event arrived before the cap keeps its success result. Manual probes and the Comparison Lab with a large `max_tokens` get max(cap, `max_tokens` / 20 tok/s) so long outputs are not cut (4096 → 205 s, 8192 → 410 s); every scheduled workload preset (≤ 512 tokens) uses exactly 90 s. Request payloads, labels and metric semantics are unchanged.
- **프로브 wall-clock watchdog.** 모든 프로브 스트림(OpenAI Responses, Anthropic CP `messages.stream`, Bedrock `converse_stream`, Comparison Lab)이 wall-clock 상한 `PROBE_WALL_CLOCK_S`(env, 기본 90초, 재시도와 backoff 포함) 안에서 돈다. 세 SDK 모두 청크 사이 대기만 제한하기 때문이다. v2.28.0 벤치 watchdog은 동작 그대로 공용 `backend/stream_watchdog.py`로 옮겼고(`gptbench`는 기존 이름을 재노출), botocore `EventStream` 아래 소켓도 shutdown할 수 있게 했다. 만료되면 스트림 소켓을 끊고 그 모델만 해당 사이클에서 오류 행 `WallClockTimeout: probe exceeded 90s wall-clock`이 되며 재시도하지 않는다. 상한 안에 종료 이벤트를 받은 스트림은 성공 결과를 유지한다. `max_tokens`가 큰 수동 프로브와 Comparison Lab은 max(상한, `max_tokens` / 20 tok/s)를 받아 긴 출력이 잘리지 않는다(4096 → 205초, 8192 → 410초). 스케줄 워크로드 프리셋(512토큰 이하)은 모두 정확히 90초다. 요청 payload, 라벨, 지표 의미는 바뀌지 않았다.
- **OpenAI client: no SDK retries, bounded timeouts.** The prober's OpenAI client now uses `max_retries=0` and `httpx.Timeout` (read 60 s, connect 10 s) instead of 600 s and 2 retries, so retries happen only in the prober's own loop. That loop now also matches the exception type name, so OpenAI 429 (`RateLimitError`, whose message is only "Error code: 429 - …") is still retried, and a retry whose backoff would pass the wall-clock budget is skipped. This applies only to the probe paths (dashboard cycle, `/api/probes/run`, Comparison Lab). The parity run used to share this client and now builds its own client with the SDK defaults (timeout 600 s, `max_retries` 2), so its behavior is unchanged: `parity.engine.classify_error` records 429/5xx/connection errors as `broken`, and the SDK retries do fire there (14 `Retrying request` lines in `/ecs/parityrun` on 2026-09-22). Only the credential choice (`prober._openai_api_key`) is shared.
- **OpenAI 클라이언트: SDK 재시도 없음, timeout 상한.** prober의 OpenAI 클라이언트는 600초와 재시도 2회 대신 `max_retries=0`과 `httpx.Timeout`(read 60초, connect 10초)을 쓴다. 재시도는 prober 자체 루프에서만 일어난다. 이 루프는 예외 타입명도 함께 보므로 OpenAI 429(`RateLimitError`, 메시지는 "Error code: 429 - …"뿐)는 계속 재시도되고, backoff 뒤 wall-clock 예산이 남지 않는 재시도는 건너뛴다. 이 설정은 프로브 경로(대시보드 사이클, `/api/probes/run`, Comparison Lab)에만 적용된다. 이 클라이언트를 함께 쓰던 패리티 런은 이제 SDK 기본값(timeout 600초, `max_retries` 2) 클라이언트를 따로 만들어 동작이 그대로다. `parity.engine.classify_error`는 429/5xx/연결 오류를 `broken`으로 기록하고, 패리티에서는 SDK 재시도가 실제로 쓰인다(2026-09-22 `/ecs/parityrun`에 `Retrying request` 14건). 공유하는 것은 자격증명 선택(`prober._openai_api_key`)뿐이다.
- **One unfinished model no longer fails or stalls the cycle.** `_await_probes` gives each model `PROBE_FUTURE_TIMEOUT_S` = max(120 s, cap + 30 s), measured from when its worker starts. Past that the cycle writes the model's error row `probe did not finish within 120s (cycle timeout)` and the run still completes, so `/api/auto-probe/latest` and `/status` advance. A 600 s cycle deadline also records models that never started because abandoned threads filled the 3-worker pool (`probe not started before the 600s cycle deadline (cycle timeout)`). A late worker's commit is turned into a rollback under a per-model lock, so there is never a duplicate `(run_id, model_id)` row. Worker exceptions other than timeouts still fail the run, as before (no empty completed cycle). The executor is shut down with `wait=False, cancel_futures=True` instead of the context-manager join; per-worker DB sessions and `max_workers=3` are unchanged.
- **끝나지 않는 모델 하나가 사이클을 실패시키거나 멈추지 않는다.** `_await_probes`는 모델마다 워커 시작 시점부터 `PROBE_FUTURE_TIMEOUT_S` = max(120초, 상한 + 30초)를 준다. 이를 넘기면 사이클이 그 모델의 오류 행 `probe did not finish within 120s (cycle timeout)`을 쓰고 run은 그대로 완료되므로 `/api/auto-probe/latest`와 `/status`가 전진한다. 포기한 스레드가 워커 3개를 모두 채워 시작조차 못한 모델은 사이클 데드라인 600초에서 기록된다(`probe not started before the 600s cycle deadline (cycle timeout)`). 늦게 끝난 워커의 커밋은 모델별 lock 아래에서 롤백으로 바뀌어 `(run_id, model_id)` 행이 중복되지 않는다. 타임아웃이 아닌 워커 예외는 이전처럼 run을 실패시킨다(빈 completed 사이클 방지). executor는 context manager join 대신 `wait=False, cancel_futures=True`로 종료한다. 워커별 DB 세션과 `max_workers=3`은 그대로다.
- **`auto_prober_runner` exits right after the cycle.** `sys.exit()` joins `ThreadPoolExecutor` workers at interpreter shutdown, so a stray blocked thread kept the task alive. The runner now disposes the DB engine, flushes logging and calls `os._exit(code)`. Exit codes are unchanged (0 = completed or skipped as overlapping, 1 = failed).
- **`auto_prober_runner`는 사이클 직후 종료한다.** `sys.exit()`는 인터프리터 종료 단계에서 `ThreadPoolExecutor` 워커를 join하므로 멈춘 스레드 하나가 태스크를 살려 두었다. 이제 러너는 DB 엔진을 정리하고 로그를 flush한 뒤 `os._exit(code)`로 끝난다. exit code 의미는 그대로다(0 = 완료 또는 겹침 skip, 1 = 실패).

### Infra
- No new required env and no IAM or CDK change; `PROBE_WALL_CLOCK_S` is optional (default 90). Deploy the new backend image with the digest-pinned CDK path (`BedrockMonitor-AppServices` + `BedrockMonitor-Scheduler`) so the autoprober task definition moves to it together with the backend service.
- 필수 신규 env, IAM, CDK 변경은 없다. `PROBE_WALL_CLOCK_S`는 선택이다(기본 90). autoprober 태스크 정의가 backend 서비스와 함께 새 이미지로 가도록 digest 고정 CDK 경로(`BedrockMonitor-AppServices` + `BedrockMonitor-Scheduler`)로 배포한다.
- **Operational note**: a task that hung before the fix keeps running on the old image until its stream unblocks. If an autoprober task is still RUNNING long after deploy while new tasks log "skipping overlapping cycle", stop it with `aws ecs stop-task` (see `docs/runbooks/troubleshooting.md`); its reservation also expires 900 s after the run was created. After the fix a stalled model shows up in the dashboard anomaly box as an error row (`WallClockTimeout: …` or `… (cycle timeout)`) instead of freezing the dashboard, and the reliability page counts it in the `network` (timeout) bucket.
- **운영 참고**: 수정 전에 멈춘 태스크는 스트림이 풀릴 때까지 옛 이미지로 계속 돈다. 배포 뒤에도 autoprober 태스크가 오래 RUNNING이고 새 태스크가 "skipping overlapping cycle"을 남기면 `aws ecs stop-task`로 멈춘다(`docs/runbooks/troubleshooting.md`). 예약은 run 생성 900초 뒤 어차피 만료된다. 수정 뒤에는 멈춘 모델이 대시보드를 동결시키지 않고 이상 징후 박스에 오류 행(`WallClockTimeout: …` 또는 `… (cycle timeout)`)으로 나타나며, 신뢰성 화면은 이를 `network`(timeout) 버킷으로 센다.

### Docs
- New `docs/runbooks/troubleshooting.md` entry "Dashboard frozen / skipping overlapping cycle" (symptoms, log strings, checks, fix); probe wall-clock watchdog notes in `CLAUDE.md` (AutoProber) and `backend/CLAUDE.md` (key files). Tests: `backend/tests/test_probe_watchdog.py`, `test_auto_prober_timeout.py`, `test_auto_prober_runner.py`, `test_stream_watchdog.py`, `test_parity_openai_client.py`. Version bumped to **v2.28.2**.
- `docs/runbooks/troubleshooting.md`를 새로 만들고 "대시보드 동결 / skipping overlapping cycle" 항목(증상, 로그 문자열, 확인, 조치)을 넣었다. `CLAUDE.md`(AutoProber)와 `backend/CLAUDE.md`(주요 파일)에 프로브 wall-clock watchdog 메모를 추가했다. 테스트는 `backend/tests/test_probe_watchdog.py`, `test_auto_prober_timeout.py`, `test_auto_prober_runner.py`, `test_stream_watchdog.py`, `test_parity_openai_client.py`다. 버전을 **v2.28.2**로 올렸다.
