...
function formatRelativeTime(isoString: string | null): string {
  ...
  if (diffSec < 0) {
    ...
    const sec = absSec % 60;
    return `${min}분 ${sec}${ko.seconds}`;
  }
  if (diffSec < 60) return ko.justNow;
  ...
  useEffect(() => {
    const id = setInterval(() => {
      if (status?.next_run_time) {
        setNextCountdown(formatCountdown(status.next_run_time));
      }
    }, 1000);
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-4">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-6">
            <h2 className="text-sm font-semibold text-gray-200">{ko.autoProbeStatus}</h2>

            {/* Last probe */}
            <div className="text-xs text-gray-400">
              <span className="text-gray-600">{ko.lastProbe}: </span>
              <span className="text-gray-300">{formatRelativeTime(status?.last_run_time ?? null)}</span>
            </div>

            {/* Next probe */}
            <div className="text-xs text-gray-400">
              <span className="text-gray-600">{ko.nextProbe}: </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-2 text-xs text-gray-500 cursor-pointer">
              ...
              />
              {ko.autoRefresh} ({countdown}{ko.seconds})
            </label>
            ...
            >
              {triggerLoading ? ko.triggering : ko.triggerNow}
            </button>
          </div>
        </div>
      </div>
      ...
      {hasData ? (
        <>
          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-4">
            <h3 className="text-sm font-semibold text-gray-200 mb-3">지표 설명</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {Object.entries(ko.metrics).map(([key, m]) => (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-300 mb-2">{ko.noDataYet}</h2>
          <p className="text-gray-500 max-w-md text-sm">{ko.noDataDesc}</p>
          {status?.current_cycle_running && (
