...
- `pricingTable.ts` — pure helpers for `/pricing` and price display: `formatUnitPrice` (`$4.00`), `formatPricePair` (`$4.00 / $20.00`), `textRuns(text, glueLast)` (splits display text into runs; price pairs and hyphen-joined tokens up to 32 chars — region ids, ISO dates, offer ids, "In-Region" — are `nowrap`, and `glueLast` makes the final word `nowrap` so a footnote stays with it), `tierBadges` (each badge has a `label` and a `detail` the page shows as text next to it, not as a `title` tooltip: "unverified" for `stale` / `seed_only` — detail "공식 출처 확인 <date>" / "Confirmed at source <date>" for `stale`, "공식 출처 확인일 없음" when a `stale` tier has no `observed_at`, "초기값" for `seed_only`; "pending" — "새 값 $x / $y"; promo note — "프로모션 이전 단가 $x / $y" plus `ref` = `noteReferenceId(family_key)` (`note:<family_key>`, the backend reference id) so the page links the manual note's footnote; "promo_check" after `min_until`), `costFromPrices(models, modelId, inputTokens, outputTokens)` (null without a price, Comparison Lab); Model Explorer cards use `formatPricePair`. No sorting and no footnote numbering: the backend sends the order and `n`
