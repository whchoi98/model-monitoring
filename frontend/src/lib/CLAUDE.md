...
- `pricingTable.ts` — pure helpers for `/pricing` and price display: `formatUnitPrice` (`$4.00`), `formatPricePair` (`$4.00 / $20.00`), `tierBadges` (each badge has a `label` and a `detail` the page shows as text next to it, not as a `title` tooltip: "unverified" for `stale` / `seed_only` — detail "마지막 확인 <date>" for `stale`, "마지막 확인일 없음" when a `stale` tier has no `observed_at`, "초기값" for `seed_only`; "pending" — "새 값 $x / $y"; promo note — "프로모션 이전 단가 $x / $y" plus `ref` = `noteReferenceId(family_key)` (`note:<family_key>`, the backend reference id) so the page links the manual note's footnote; "promo_check" after `min_until`), `costFromPrices(models, modelId, inputTokens, outputTokens)` (null without a price, Comparison Lab); Model Explorer cards use `formatPricePair`. No sorting and no footnote numbering: the backend sends the order and `n`
