/**
 * 대시보드 모델 카드 지표 등급 (v2.28.0) — TTFT, 총 응답시간, TPS 값을 정상/경고/위험으로 분류.
 *
 ...
 * 정했다(사용자 승인 설계). 같은 48시간 데이터에 적용한 시뮬레이션 분포는 정상 89%, 경고 9.4%,
 * 위험 1.4%다. 기준을 바꿀 때는 이 표만 수정한다(카드 색, 툴팁, 범례 기준표가 모두 여기서 읽는다).
 *
