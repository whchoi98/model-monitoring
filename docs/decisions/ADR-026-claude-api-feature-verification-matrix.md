...
1. **형제 패키지 `backend/claude_features/`** — 패리티는 그대로 두고 순수 함수만 공유. 카탈로그는 39행
   (문서 33피처 + 코어 4 + Models API·strict_tool_use/extended_thinking 분할 2), 행마다
   `documented{cp,mantle,bedrock_invoke,bedrock_converse}`와 검증 강도(`evidence|acceptance|capability|negative`)를 명시.
...
로컬 라이브 스모크(`sonnet-5` 전체 39피처 × 4 surface 스윕, task-7-report 참조)에서 확인된 드리프트와 미문서화 동작:
...
- **Bedrock InvokeModel/Converse가 Claude 5 세대에서 structured outputs·strict tool use를 거부** — `output_config.format`,
  ...
  낙관적 서술이었다 — 카탈로그 `documented`를 두 열 모두 `no`로 낮춰 `match`로 정정.
- **Bedrock `CountTokens`가 CRIS 전용(`global.*`) Claude 5 모델을 지원하지 않음** — 프로파일 형태(`global.` 접두) 문제가
  ...
  카탈로그를 `{cp: ga, mantle: ga, bedrock_invoke: no, bedrock_converse: no}`로 정정.
- **`browser_toolset_20260801`이 문서에 없는데 동작** — CP on AWS·Bedrock InvokeModel 둘 다 toolset을 수락하고
...
- (−) 런당 토큰 비용 대략 $4~6(월 ~$120~180, Fable 지배) — 1런 = 프로브 506 + 사전판정 118 = 624셀, 캐싱·부정 제어
  포함 ≈650 API 호출. MCP 프로브는 공개 MCP 서버 의존(장애는 inconclusive로 격리).
