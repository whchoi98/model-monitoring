...
EventBridge Scheduler
   ...
   ├─ rate(15 minutes)   → ECS RunTask "gptbench"   → Mantle GPT 12채널 TTFB/TTFT 벤치 → RDS
...
| GptBench TaskDef | `python -m gptbench_runner --once` — Mantle GPT 12채널 TTFB/TTFT 벤치 (GPT 5.4/5.5/5.6 Terra + GPT 6 Astra Global, US CRIS, us-west-2 — v2.25.1) |
...
EventBridge Scheduler
   ...
   ├─ rate(15 minutes)   → ECS RunTask "gptbench"   → Mantle GPT 12-channel TTFB/TTFT bench → RDS
