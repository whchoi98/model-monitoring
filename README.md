...
[![Version](https://img.shields.io/badge/version-2.23.1-blue.svg)](CHANGELOG.md)
...
Amazon Bedrock LLM Monitor is a production-grade observability platform that continuously probes 43 LLM channels across Bedrock Global / US inference profiles, Anthropic CP on AWS, and OpenAI GPT via Bedrock Mantle (Path 4) plus Bedrock Global cross-region profiles for GPT-5.6 (v2.20.0). (An OpenAI 1P direct path — Path 5 — exists in code but is dormant/hidden as of v2.19.1.) It surfaces latency (TTFT, total, server), throughput (TPS), output token distribution, stop-reason patterns, multi-channel reliability, and 30-day cost projections — all behind a Next.js dashboard with six analytical views.
...
# Inspect the latest 43-model probe results
...
Amazon Bedrock LLM Monitor는 Bedrock Global / US 추론 프로파일, Anthropic CP on AWS, OpenAI GPT via Bedrock Mantle(Path 4) 및 GPT-5.6 Bedrock Global cross-region 프로파일(v2.20.0)을 포함한 43개 LLM 채널을 지속적으로 프로빙하여 지연(TTFT, 총 응답시간, 서버 처리시간), 처리량(TPS), 출력 토큰 분포, 정지 사유 패턴, 다중 채널 신뢰성, 30일 비용 예측을 한 대시보드에서 제공하는 운영 등급 관측 플랫폼입니다.
