...
- `lib/stacks/scheduler-stack.ts` — 5 schedules (AutoProber and Insights `rate(5 minutes)`, ParityRun 12 h, GptBench 15 min, FeaturesVerify `cron(30 17 * * ? *)` UTC); `buildTaskDef` reuses the backend image with a CMD override (512 CPU / 1024 MiB, ARM64); `ANTHROPIC_CP_PROBE_INTERVAL_S=600` only on the AutoProber task
