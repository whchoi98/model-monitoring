...
## Files
- `catalog.py` — `SURFACES` 6개 (converse / invoke_model / messages / messages_mantle / chat_completions / responses), `FEATURES` 12개 (basic, streaming, system_instructions, tool_use, structured_output, reasoning, caching + v2.14.0: adaptive_thinking, count_tokens, batches, web_search, computer_use — `_FEATURE_SURFACES` 맵으로 surface 제한). `surfaces_for(model_id)` — model_id 접두사로 surface 결정 (Bedrock Claude는 messages_mantle 포함), `mantle_fm_id()` — 프로파일 접두사 제거, `is_applicable()` — reasoning은 `_REASONING_MARKERS` 모델만
