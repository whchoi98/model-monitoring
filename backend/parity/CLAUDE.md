...
- **messages_mantle** (v2.13.0): Bedrock Mantle `/anthropic` 엔드포인트 시험 — `aws-bedrock-token-generator`의 SigV4 파생 bearer + FM id, 리전 `MANTLE_ANTHROPIC_REGION` — env 미주입 시 코드 폴백은 여전히 `ap-northeast-1`이지만, CDK가 backend와 모든 스케줄 태스크에 `us-east-1`을 명시 주입한다(2026-09-05 결정 — `ap-northeast-1`은 `/anthropic`에서 Opus 4.8만 서빙). 따라서 배포 환경의 `messages_mantle`과 Claude API Features Mantle 열은 둘 다 `us-east-1`을 프로빙한다. 2026-07-11 실측(ap-northeast-1 기준): 엔드포인트는 실존하나 서빙 모델 없음 → 전량 "명시적 미지원"(`does not exist` → unsupported)
