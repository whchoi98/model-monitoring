...
FEATURES: list[dict] = [
    {
        ...
        "label_en": "Basic response",
        "desc_en": "Non-streaming single request — verifies a non-empty text response",
    },
    {
        "id": "streaming",
        "label_ko": "스트리밍",
        ...
        "label_en": "Streaming",
        "desc_en": "Stream request — verifies 2+ content delta events are received",
    },
    {
        "id": "system_instructions",
        "label_ko": "시스템 지시",
        "desc_ko": "system 필드로 카나리 단어 강제 — 응답에 카나리가 실제 반영되는지 검증",
        "label_en": "System instructions",
        "desc_en": "Forces a canary word via the system field — verifies the canary appears in the response",
    },
    {
        "id": "tool_use",
        "label_ko": "도구 호출",
        ...
        "label_en": "Tool use",
        "desc_en": "Forces the echo tool (tool_choice) — verifies the canary argument round-trips through the tool call",
    },
    {
        "id": "structured_output",
        "label_ko": "구조화 출력",
        "desc_ko": "JSON 출력 강제 — 필수 키를 가진 유효한 JSON인지 검증",
        "label_en": "Structured output",
        "desc_en": "Forces JSON output — verifies valid JSON with the required key",
    },
    {
        "id": "reasoning",
        "label_ko": "확장 추론",
        "desc_ko": "thinking/reasoning 활성 — 추론 토큰 사용 또는 추론 블록 존재를 검증",
        "label_en": "Extended reasoning",
        "desc_en": "Enables thinking/reasoning — verifies reasoning tokens or a reasoning block",
    },
    {
        "id": "caching",
        "label_ko": "프롬프트 캐싱",
        "desc_ko": "동일 프롬프트 반복 — 두 번째 응답의 cached tokens > 0 검증",
        "label_en": "Prompt caching",
        "desc_en": "Repeats the same prompt — verifies cached tokens > 0 on the second response",
    },
    # --- v2.14.0 확장 5종 ---
    {
    ...
    },
    {
        ...
        "label_en": "Count tokens",
        "desc_en": "Count Tokens endpoint round-trip — verifies input_tokens > 0",
    },
    {
        "id": "batches",
        "label_ko": "배치",
        ...
        "label_en": "Batches",
        "desc_en": "Message Batches submit → status round-trip — verifies creation and status lookup (cancelled afterwards)",
    },
    {
        "id": "web_search",
        "label_ko": "웹 검색 도구",
        "desc_ko": "서버측 web_search 도구 정의 수락 검증 — 도구 포함 요청이 정상 완료되는지",
        "label_en": "Web search tool",
        "desc_en": "Verifies acceptance of the server-side web_search tool definition — request completes normally",
    },
    {
        "id": "computer_use",
        "label_ko": "컴퓨터 사용 도구",
        "desc_ko": "computer use 도구 정의 수락 검증 (beta 헤더 포함)",
        "label_en": "Computer use tool",
        "desc_en": "Verifies acceptance of the computer use tool definition (with beta header)",
    },
    # --- v2.15.0 확장 7종 (참조 도구 수준) ---
    {
        ...
        "label_en": "Reasoning effort",
        "desc_en": "Verifies acceptance of the effort parameter (low/high) — Claude output_config, GPT reasoning effort",
    },
    {
        "id": "json_schema",
        "label_ko": "JSON 스키마 강제",
        "desc_ko": "strict JSON schema 출력 강제 — 필수 키를 가진 유효 JSON 검증",
        "label_en": "JSON schema",
        "desc_en": "Forces strict JSON schema output — verifies valid JSON with the required key",
    },
    {
        "id": "url_sources",
        "label_ko": "URL 문서 소스",
        "desc_ko": "document source type: url — 원격 PDF를 컨텍스트로 수락하고 응답하는지 검증",
        "label_en": "URL document source",
        "desc_en": "document source type: url — verifies a remote PDF is accepted as context and answered",
    },
    {
        "id": "memory_tool",
        "label_ko": "메모리 도구",
        "desc_ko": "memory 도구 정의 수락 검증 (beta 헤더 포함)",
        "label_en": "Memory tool",
        "desc_en": "Verifies acceptance of the memory tool definition (with beta header)",
    },
    {
        "id": "code_execution",
        "label_ko": "코드 실행 도구",
        "desc_ko": "서버측 code_execution 도구 정의 수락 검증 (beta 헤더 포함)",
        "label_en": "Code execution tool",
        "desc_en": "Verifies acceptance of the server-side code_execution tool definition (with beta header)",
    },
    {
        "id": "files_api",
        "label_ko": "Files API",
        ...
        "label_en": "Files API",
        "desc_en": "File list endpoint round-trip — verifies the Files API is offered",
    },
    {
        "id": "models_api",
        "label_ko": "Models API",
        ...
        "label_en": "Models API",
        "desc_en": "Model retrieve endpoint round-trip — verifies the model id is retrievable",
    },
]
