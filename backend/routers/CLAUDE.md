...
- `features.py` — `/api/features/*` — catalog (39행×4 surface 정의), latest (완료 런 매트릭스 + 직전 런 대비 diff + drift), evidence (셀별 요청 스냅샷·응답 신호), trigger (JWT, backend 내 백그라운드 스레드) (v2.23.0)
