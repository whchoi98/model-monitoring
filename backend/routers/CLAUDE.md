...
- `features.py` — `/api/features/*` — catalog (39행×5 surface 정의), latest (완료 런 매트릭스 + 직전 런 대비 diff — `kind: catalog|measured`로 카탈로그 규칙 변경과 실측 변경 구분(신규 셀·사전판정 행 = catalog), v2.24.0 + drift), evidence (셀별 요청 스냅샷·응답 신호), trigger (JWT, backend 내 백그라운드 스레드) (v2.23.0)
